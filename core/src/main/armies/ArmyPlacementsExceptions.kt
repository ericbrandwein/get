...
class NonPositiveAmountAddedException(val amount: Int) :
    IllegalArgumentException(
        "Can't add the non-positive amount $amount of armies to a country.")

class CantOccupyWithNoArmiesException(val amount: Int) :
    IllegalArgumentException(
        "Can't occupy a country with non-positive amount of armies $amount.")
...
class CantRemoveMoreArmiesThanAvailableException(
    ...
    "Can't remove $amountRemoved armies from country $country with $originalAmount " +
        "armies.")

class NonPositiveAmountRemovedException(val amountRemoved: Int) :
    IllegalArgumentException(
        "Can't remove non-positive amount of armies $amountRemoved from a country.")
